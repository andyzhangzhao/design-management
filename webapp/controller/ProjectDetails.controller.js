    ...
    function (Controller, JSONModel, Filter) {
        ...
        return Controller.extend("projectmanagement.controller.ProjectDetails", {
            onInit: function () {
                ...
                this.oDetailsItemsModel = this.getOwnerComponent().getModel('detailsItems');
                        ...
                        response.results.forEach(function (item) {
                            if (item.Sid.length === 1) {
                                ...
                                if (aItems[aItems.length - 1].items) {
                                    aItems[aItems.length - 1].items.push({ title: item.Sname, key: item.Sid })
                                } else {
                                    aItems[aItems.length - 1].items = [{ title: item.Sname, key: item.Sid }]
                                }
                            }
                        })
            ...
            _onObjectMatched: function (oEvent) {
                ...
                this.oProjectListModel.read("/zrre_c_dmcp", {
                    ...
                    success: function (response) {
                        this.oView.getModel('ui').setProperty('/projectTitle', response.results[0].Dspnm)
